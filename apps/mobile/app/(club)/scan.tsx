...
export default function ScanScreen() {
  ...
  const processBooking = async (code: string, bookingId?: string) => {
    ...
    try {
      ...
      if (booking.status !== BookingStatus.CONFIRMED && booking.status !== BookingStatus.PENDING) {
        showError('Ogiltig bokning', 'Denna bokning har redan använts eller är inte giltig');
        setScanned(false);
        setIsLoading(false);
        return;
      }
      ...
      const className = `för ${completedBooking.classes?.name || 'för träningspass'}`;
      const userName =
        completedBooking.profiles?.display_name ||
        completedBooking.profiles?.first_name ||
        'Medlem';

      showSuccess('Incheckning Godkänd!', `${userName} är incheckad ${className}. Välkommen!`);
      ...
      showError('Incheckningsfel', err.message || 'Kunde inte genomföra incheckning. Försök igen.');
      setScanned(false);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleBarCodeScanned = async ({ type, data }: { type: string; data: string }) => {
    ...
    try {
      ...
      showError('QR-kod Fel', err.message || 'Kunde inte läsa QR-koden. Försök igen.');
      setScanned(false);
    }
  };
        ...
        <View className="flex-1 justify-center items-center bg-background px-6">
          ...
          {!permission.canAskAgain && (
            ...
            >
              ...
              <Text className="text-textSecondary font-medium ml-2">Öppna Inställningar</Text>
            </TouchableOpacity>
          )}
        </View>
      </SafeAreaWrapper>
      ...
      >
        <View className="flex-1 p-4">
          ...
          {!manualMode ? (
            <>
              <View className="flex-1 rounded-2xl overflow-hidden mb-4 bg-black relative">
                ...
                {scanned && (
                  <View className="absolute inset-0 bg-black/50 items-center justify-center">
                    <Text className="text-white text-lg font-bold">Bearbetar...</Text>
                  </View>
                )}
              </View>
              ...
              <TouchableOpacity
                ...
                disabled={isLoading}
              >
                <Text className="text-textPrimary font-semibold ml-2">Ange kod manuellt</Text>
              </TouchableOpacity>
            </>
          ) : (
            <>
              ...
              <View className="flex-1 justify-center">
                <View className="bg-surface rounded-2xl p-6 mb-4">
                  <Text className="text-textSecondary text-sm mb-2 text-center">Bokningskod</Text>
                  <TextInput
                ...
                <TouchableOpacity
                  className={`rounded-2xl py-4 mb-4 ${
                    bookingCode.length === 6 && !isLoading ? 'bg-primary' : 'bg-surface'
                  }`}
                  onPress={handleManualSubmit}
                  disabled={bookingCode.length !== 6 || isLoading}
                  style={{
                    shadowColor: bookingCode.length === 6 ? colors.primary : 'transparent',
                    shadowOffset: { width: 0, height: 4 },
                    shadowOpacity: 0.3,
                    shadowRadius: 8,
                ...
                >
                  <Text
                    className={`text-center font-bold text-lg ${
                      bookingCode.length === 6 ? 'text-white' : 'text-textSecondary'
                    }`}
                  >
                ...
                <TouchableOpacity
                  ...
                  disabled={isLoading}
                >
                  <Text className="text-textSecondary font-semibold">Tillbaka till kamera</Text>
                </TouchableOpacity>
              </View>
            </>
          )}
          ...
          {!manualMode && scanned && (
            ...
            >
              <Text className="text-white text-center font-semibold text-lg">Skanna Igen</Text>
            </TouchableOpacity>
          )}
        </View>
      </KeyboardAvoidingView>
