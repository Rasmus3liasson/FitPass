...
import { Calendar, ChatCircle } from 'phosphor-react-native';
...
export default function SocialScreen() {
  ...
  const { data: clubClasses = [] } = useClassesByClub(selectedClubForClasses?.id || '');
  ...
  const socialClasses = allClasses
    ...
    .map((classItem) => ({
      ...
      duration: Math.floor(
        (new Date(classItem.end_time).getTime() - new Date(classItem.start_time).getTime()) /
          (1000 * 60)
      ),
      participants: {
  ...
  const filteredNews = useMemo(() => {
    ...
    return activeNewsData.filter((news) => news.club_id && favoriteClubIds.includes(news.club_id));
    ...
    <SafeAreaWrapper edges={['top']} className="bg-background">
      <AnimatedScreen>
        <PageHeader
          ...
          rightElement={
            <TouchableOpacity onPress={() => router.push(ROUTES.MESSAGES as any)}>
          ...
          <>
            ...
            <View className="flex-row bg-surface/50 rounded-xl mx-4 mb-4 p-1">
              ...
              >
                <Text
                  className={`ml-2 font-medium ${
                    newsFilter === 'all' ? 'text-textPrimary' : ' text-textSecondary'
                  }`}
                >
                  Alla Nyheter
                </Text>
              ...
              >
                <Text
                  className={`ml-2 font-medium ${
                    newsFilter === 'favorites' ? 'text-textPrimary' : 'text-textSecondary'
                  }`}
                >
                  Endast Favoriter
                </Text>
            ...
            {newsLoading || newsTableLoading ? (
              ...
              <NewsletterFeed newsItems={newsItems} onNewsItemPress={handleNewsItemPress} />
            )}
          </>
        )}
