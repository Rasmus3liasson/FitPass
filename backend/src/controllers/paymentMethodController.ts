...
export class PaymentMethodController {
  ...
  async createPaymentMethod(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { customerId, cardNumber, expMonth, expYear, cvc, isUserAdded, userId, email, name }: CreatePaymentMethodRequest = req.body;
      ...
      if (!actualCustomerId) {
        if (!userId || !email) {
          sendErrorResponse(res, "Customer ID is required, or userId and email must be provided to create a new customer", undefined, 400);
          return;
        }
  ...
  async getUserPaymentMethods(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.log("📊 Found payment methods count:", paymentMethods.data.length, "hasReal:", hasRealPaymentMethods);
      ...
      if (!hasRealPaymentMethods) {
        console.log("⚠️  UI ALERT: User should see 'Add Card Information' prompt because hasRealPaymentMethods=false");
      } else {
        console.log("✅ UI SUCCESS: User should NOT see 'Add Card Information' prompt because hasRealPaymentMethods=true");
      }
