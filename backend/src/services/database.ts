...
export class DatabaseService {
  ...
  async createOrUpdateMembership(membershipData: {
    ...
    if (existingMembership) {
      ...
      const isProduction =
        membershipData.is_production || process.env.EXPO_PUBLIC_ENVIRONMENT === 'production';
      ...
      return await this.createMembership({
        ...
        start_date: membershipData.start_date || new Date().toISOString(),
        end_date:
          membershipData.end_date || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        is_active: true,
  ...
  async getMembershipBySubscriptionId(stripeSubscriptionId: string): Promise<any> {
    const { data, error } = await supabase
      .from('memberships')
      .select('*')
      .eq('stripe_subscription_id', stripeSubscriptionId)
