...
export class StripeService {
  ...
  async syncProductsWithDatabase(): Promise<void> {
    ...
    for (const plan of membershipPlans) {
      try {
        ...
        await dbService.updateMembershipPlanStripeIds(plan.id, stripeProduct.id, stripePrice.id);
    ...
    return products.data.map((product) => ({
      ...
      default_price: product.default_price
        ? {
            ...
            unit_amount:
              typeof product.default_price === 'string' ? null : product.default_price.unit_amount,
            currency:
              typeof product.default_price === 'string' ? 'sek' : product.default_price.currency,
          }
        : null,
    }));
  }
  ...
  async handleWebhook(body: Buffer, signature: string): Promise<void> {
    ...
    if (!webhookSecret) {
      throw new Error(`Webhook secret not configured for ${process.env.NODE_ENV} environment`);
    }
  ...
  private async handleSubscriptionUpdated(subscription: Stripe.Subscription): Promise<void> {
    ...
    if (!subscription.current_period_start || !subscription.current_period_end) {
      console.log(
        '⚠️ Webhook subscription object incomplete, fetching full subscription from Stripe...'
      );
      try {
  ...
  private async syncSubscriptionToDatabase(subscription: Stripe.Subscription): Promise<void> {
    ...
    try {
      ...
      if (existingMembership) {
        ...
        const planChanged =
          existingMembership.plan_id !== plan.id || existingMembership.stripe_price_id !== priceId;

        if (planChanged) {
          console.log(`📋 Plan change detected: ${existingMembership.plan_type} → ${plan.title}`);
          console.log(
            `   Old credits: ${existingMembership.credits}, New credits: ${plan.credits}`
          );
  ...
  async calculatePaymentCuts(
    ...
    if (planType === 'tiered') {
      ...
      const cutPerGym = unlimitedCuts[gymCount.toString()] || unlimitedCuts['3']; // Default to 3+ gym rate
    ...
    try {
      ...
      for (const gymCut of payment.gym_cuts) {
        ...
        if (error || !gym?.stripe_account_id) {
          console.error(`❌ Gym ${gymCut.gym_id} does not have Stripe Connect account`);
          continue;
        }
