...
export async function runMigrations() {
  try {
    ...
    try {
      const { error: testError } = await supabase.from('profiles').select('push_token').limit(1);
    ...
    try {
      ...
      if (triggerError || !triggers || triggers.length === 0) {
        ...
        console.log('   File: backend/src/sql/update_class_booked_spots_triggers.sql');
        console.log('   This will automatically sync booked_spots when bookings change');
