...
async function testConfigAdaptation() {
  ...
  try {
    ...
    if (newValue === testPrice) {
      ...
      throw new Error(
        `Config did not adapt! Expected ${testPrice}, got ${newValue}`,
      );
    }
    ...
    console.log(
      "  Cache age: " +
        Math.floor((Date.now() - newConfig.lastFetched.getTime()) / 1000) +
        "s",
    );
    ...
    if (restoredValue === originalPrice) {
      ...
      console.log(
        `  ⚠️  Config shows ${restoredValue} SEK, expected ${originalPrice} SEK`,
      );
    }
