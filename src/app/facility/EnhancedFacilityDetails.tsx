...
const getCurrentStatus = (openHours: Record<string, string>) => {
  ...
  if (!todayHours || todayHours.toLowerCase() === 'closed') {
    return { isOpen: false, status: 'Closed', nextOpen: getNextOpenTime(openHours, now), isClosingSoon: false };
  }
  
  // Parse hours (assume format like "06:00-22:00")
  const [openTime, closeTime] = todayHours.split('-');
  if (!openTime || !closeTime) {
    return { isOpen: false, status: 'Closed', nextOpen: null, isClosingSoon: false };
  }
  ...
  if (currentTime >= openMinutes && currentTime < closeMinutes) {
    ...
    if (minutesUntilClose <= 60) {
      return { isOpen: true, status: `Closes in ${minutesUntilClose}min`, isClosingSoon: true, nextOpen: null };
    }
    return { isOpen: true, status: `Open until ${closeTime}`, isClosingSoon: false, nextOpen: null };
  }
...
const getNextOpenTime = (openHours: Record<string, string>, currentDate: Date) => {
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (hours && hours.toLowerCase() !== 'closed') {
      ...
      return i === 1 ? `Tomorrow at ${openTime}` : `${dayNames[checkDay]} at ${openTime}`;
    }
  }
  return null;
};

export function EnhancedFacilityDetails({ facility, club, onViewOnMap }: Props) {
  ...
  // Get status info
  const statusInfo = club?.open_hours ? getCurrentStatus(club.open_hours) : { isOpen: false, status: 'Hours vary', nextOpen: null, isClosingSoon: false };
    ...
    <View>
      ...
      <View className="mb-6">
        ...
        <View className="flex-row justify-between items-start mb-3">
          ...
          {/* Status Indicator */}
          <View className={`rounded-full px-3 py-2 flex-row items-center ${
            statusInfo.isOpen 
      ...
      <View className="bg-surface rounded-2xl p-4 mb-6">
        ...
        <Text className={`text-sm font-medium ml-8 mb-3 ${
          statusInfo.isOpen 
            ? statusInfo.isClosingSoon 
              ? 'text-orange-400' 
              : 'text-green-400'
          ...
          {showAllHours && (
            <View className="ml-8">
              {formatOpeningHours().map((day, index) => (
                <View key={index} className={`flex-row justify-between items-center py-2 px-3 rounded-lg mb-1 ${
