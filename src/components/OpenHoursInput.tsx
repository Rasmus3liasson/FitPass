...
export default function OpenHoursInput({ value, onChange }: {
  ...
  return (
    <View className="mb-4">
      <Text className="text-white font-semibold mb-2">Open Hours</Text>
      {days.map((day) => {
        const [open, close] = (openHours[day] || "08:00-20:00").split("-");
        return (
          <View key={day} className="flex-row items-center mb-2">
            ...
            <TextInput
              className="bg-surface text-white rounded-lg px-2 py-1 border border-borderGray w-20 mr-2"
              value={open}
            ...
            <TextInput
              className="bg-surface text-white rounded-lg px-2 py-1 border border-borderGray w-20"
              value={close}
