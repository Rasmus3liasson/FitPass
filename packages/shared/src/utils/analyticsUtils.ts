...
export const calculateAnalyticsMetrics = (
  ...
  const estimatedRevenue = revenueData ? revenueData.visits.length * pricePerVisit : 0;
  ...
  const currentPeriodRevenue = currentVisits.length * pricePerVisit;
  const previousPeriodRevenue = previousVisits.length * pricePerVisit;
