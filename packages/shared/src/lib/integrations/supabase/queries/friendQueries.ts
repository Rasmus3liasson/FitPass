...
export async function getMemberProfiles(
  ...
  const { data, error } = await query.order('created_at', { ascending: false }).limit(50); // Limit to 50 results for performance
...
export async function getFilteredMemberProfiles(
  ...
  const filteredProfiles = profiles.filter((profile) => !pendingConnections.has(profile.id));
