...
export const ScheduledChangeCard: React.FC<ScheduledChangeCardProps> = ({
  ...
  const timeUntilChange = scheduledChangeService.formatNextBillingDate(
    scheduledChange.nextBillingDate
  );
    ...
    <View className={`bg-white rounded-xl p-4 border border-gray-200 shadow-sm ${className}`}>
      ...
      <View className="flex-row items-center justify-between mb-3">
        <View className="flex-row items-center">
          ...
          <Text className="ml-2 text-lg font-semibold text-gray-900">Plan Change Scheduled</Text>
        </View>
      ...
      {/* Status Message */}
      <Text className="text-sm text-gray-600 text-center mb-4">{statusMessage}</Text>
      ...
      {scheduledChange.status === 'confirmed' && onCancel && (
        ...
        >
          <Text className="text-red-600 text-center font-medium">Cancel Scheduled Change</Text>
        </TouchableOpacity>
      )}
