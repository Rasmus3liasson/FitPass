...
export function PageHeader({
  ...
  const getTitleStyles = () => {
    ...
    const colorStyles = variant === 'gradient' ? 'text-white' : 'text-textPrimary';
    return `${baseStyles} ${colorStyles} ${titleClassName}`;
  };

  const getSubtitleStyles = () => {
    const baseStyles = 'text-sm mt-1';
    const colorStyles = variant === 'gradient' ? 'text-white/80' : 'text-textSecondary';
          ...
          <View className="flex-row items-center">
            {showBackButton && (
              <TouchableOpacity
                ...
                style={{
                  backgroundColor:
                    variant === 'gradient' ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.05)',
                }}
              >
                <ArrowLeft size={20} color={variant === 'gradient' ? 'white' : 'black'} />
              </TouchableOpacity>
            )}
            {leftElement}
          </View>
            ...
            <TouchableOpacity onPress={avatar.onPress} className="ml-3">
              <Image
                ...
                style={{
                  ...
                  borderColor: variant === 'gradient' ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.1)',
                }}
              />
            </TouchableOpacity>
          )}
