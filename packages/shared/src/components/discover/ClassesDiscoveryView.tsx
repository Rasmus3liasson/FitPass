...
export function ClassesDiscoveryView({ classes, onClassPress }: ClassesDiscoveryViewProps) {
  const now = new Date();
  ...
  const todayClasses = upcomingClasses.filter((c) => isToday(new Date(c.startTimeISO)));
  const tomorrowClasses = upcomingClasses.filter((c) => isTomorrow(new Date(c.startTimeISO)));
  const laterClasses = upcomingClasses.filter(
    (c) => !isToday(new Date(c.startTimeISO)) && !isTomorrow(new Date(c.startTimeISO))
  );
      ...
      >
        ...
        <View className="flex-row justify-between items-start mb-3">
          <View className="flex-1 mr-3">
            <Text className="text-textPrimary font-bold text-base mb-1">{classItem.name}</Text>
        ...
        {classItem.instructor && (
          <View className="flex-row items-center mt-2 pt-2 border-t border-borderGray/10">
            <Text className="text-textSecondary text-xs">
              Instruktör:{' '}
              <Text className="text-textPrimary font-medium">{classItem.instructor}</Text>
            </Text>
          </View>
        )}
      </SmoothPressable>
  ...
  const renderSection = (title: string, classes: UIClass[], emptyMessage: string) => {
    if (classes.length === 0) return null;

    return (
      <View className="mb-6">
        <Text className="text-textPrimary font-bold text-lg mb-3 px-6">{title}</Text>
        <View className="px-6">{classes.map((classItem) => renderClassCard(classItem))}</View>
      </View>
    );
  };
