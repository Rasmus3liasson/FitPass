  ...
  const calculateFilterResultCount = (tempFilters: any) => {
    ...
    const filteredResults = searchResults.filter((club) => {
      ...
      if (tempFilters.categories.length > 0) {
        const hasMatchingCategory = tempFilters.categories.some((categoryId: string) => {
          // Safety check for undefined categoryId
          if (!categoryId) return false;

          // Find the category object by ID
          const category = categories.find(
            (cat) => (cat.id && cat.id === categoryId) || (cat.name && cat.name === categoryId)
          );
          ...
          const isMatch =
            ...
            (categoryName === 'health' && (clubType === 'gym' || clubType === 'fitness')) ||
            (categoryName === 'wellness' && (clubType === 'spa' || clubType === 'fitness'));
      ...
      if (tempFilters.amenities.length > 0) {
        ...
        const hasMatchingAmenity = tempFilters.amenities.some((amenityId: string) => {
          ...
          const amenity = amenities.find(
            (a) => (a.id && a.id === amenityId) || (a.name && a.name === amenityId)
          );

          if (amenity && amenity.name && clubAmenities.includes(amenity.name)) {
            return true;
          }
      ...
      if (tempFilters.distance !== 999999 && location && club.latitude && club.longitude) {
      ...
      if (tempFilters.hasClasses) {
        ...
        const hasClasses = club.type !== 'restaurant' && club.type !== 'spa';
